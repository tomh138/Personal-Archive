...
def enhancedFeatureExtractorDigit(datum):
    ...
    ##
    """
    feat = basicFeatureExtractorDigit(datum)
    pix = datum.getPixels()
    q1xq2 = 0  # top half
    q1xq4 = 0  # right half
    numNotZero = 0
...
def contestFeatureExtractorDigit(datum):
  ...
  """
    ...
    while yBound < 29:
        ...
        while xBound < 29:
            count = 0
            # now we iterate through the sub-square and count the number of filled in pixels
            for x in range(xBound - 7, xBound):
                for y in range(yBound - 7, yBound):
                    if datum.getPixel(x, y) > 0:
                        count += 1
            percentage = (count * 100) / 49  # percentage of filled in pixels per sub-square
            features[(
                (xBound / 7) - 1, (yBound / 7) - 1)] = percentage  # fills in the features 2d array, 16 values in total
            xBound += 7
        yBound += 7
    return features
...
def runClassifier(args, options):
    ...
    nower = time.time()
    nowest = nower - now
    print("Training took " + str(nowest) + " seconds...")
    print("Validating...")
